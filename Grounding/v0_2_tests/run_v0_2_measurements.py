#!/usr/bin/env python3
"""Run v0.2 idempotence and round-trip measurement suite.

This suite does not patch doctrine. It measures what the current v0.2
extractor and committed artifacts make observable for FCL-002's
idempotence and round-trip surfaces.
"""
...
COMMITTED_DIR = REPO / "Grounding" / "v0_2"
...
OUTPUT_FILES = [
    ...
    "run_manifest.json",
    "id_crosswalk.json",
    "ClaimInventory.POPULATED.v0_2.md",
    "extract_v0_2.py",
]
...
def snapshot_output_dir(path: Path) -> dict:
    ...
    if missing:
        raise FileNotFoundError(f"Missing expected v0.2 outputs in {path}: {missing}")
...
def load_extractor_module():
    spec = importlib.util.spec_from_file_location("dme_v0_2_measure_extractor", EXTRACTOR_PATH)
...
def build_nonfatal_validate(module):
    def nonfatal_validate(entries_v0_2, crosswalk):
        live_ids = {entry["claim_id"] for entry in entries_v0_2}
        dangling = []
        resolutions = {}
        for claim_id in module.load_fcl_001_refs():
    ...
    return nonfatal_validate


def run_extractor_to(output_dir: Path, *, validation_mode: str) -> dict:
    ...
    if validation_mode == "nonfatal":
        module.validate_fcl_refs = build_nonfatal_validate(module)
...
def attempt_strict_run(output_dir: Path) -> dict:
    try:
        snapshot = run_extractor_to(output_dir, validation_mode="strict")
...
def classify_odqs(committed: dict, committed_vs_a: dict, a_vs_b: dict) -> dict:
    odq4 = {
        ...
        "measurable_surfaces": [
            ...
            f"Current v0.2 support-status distribution is measurable: {committed['support_counts']}.",
    ...
    odq5 = {
        ...
        "proxy_results": {
            ...
            "baseline_drift_field_delta_count": committed_vs_a["field_delta_count"],
...
def build_suite_report(committed: dict, strict_attempt: dict, rerun_a: dict, rerun_b: dict) -> dict:
    ...
    idempotence = {
        ...
        "strict_current_corpus_rerun": {
            "classification": "blocked" if strict_attempt["status"] != "passed" else "measurable",
            "passed": strict_attempt["status"] == "passed",
            "error": strict_attempt["error"],
        },
        "provisional_current_corpus_rerun": {
            "classification": "measurable",
            "passed": a_vs_b["all_output_hashes_equal"] and not a_vs_b["support_inflations"],
        },
    ...
    round_trip_proxies = {
        "status_fixed_point": {
            "classification": "measurable",
            "passed": a_vs_b["entry_projection_hash_equal"],
            "evidence": {
                "rerun_a_vs_rerun_b_field_delta_count": a_vs_b["field_delta_count"],
                "baseline_drift_field_delta_count": committed_vs_a["field_delta_count"],
            },
        ...
        "counit_like_non_inflation": {
            "classification": "partially_measurable",
            "passed": not a_vs_b["support_inflations"],
            "evidence": {
                "rerun_a_vs_rerun_b_support_inflations": a_vs_b["support_inflations"],
                "baseline_drift_support_inflations_uninterpreted": committed_vs_a["support_inflations"],
    ...
    return {
        "suite": {
            ...
            "source_snapshot": committed["manifest"]["source_snapshot"],
        ...
        "odq_measurability": odq_measurability,
    }
...
def markdown_report(report: dict) -> str:
    lines = [
        ...
        f"provisional_current_corpus_rerun: {report['idempotence']['provisional_current_corpus_rerun']['classification']}",
        f"provisional_current_corpus_passed: {str(report['idempotence']['provisional_current_corpus_rerun']['passed']).lower()}",
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="dme_v0_2_test_a_") as tmp_a, tempfile.TemporaryDirectory(
        prefix="dme_v0_2_test_b_"
    ) as tmp_b, tempfile.TemporaryDirectory(prefix="dme_v0_2_test_strict_") as tmp_strict:
        strict_attempt = attempt_strict_run(Path(tmp_strict) / "v0_2")
        rerun_a = run_extractor_to(Path(tmp_a) / "v0_2", validation_mode="nonfatal")
        rerun_b = run_extractor_to(Path(tmp_b) / "v0_2", validation_mode="nonfatal")
