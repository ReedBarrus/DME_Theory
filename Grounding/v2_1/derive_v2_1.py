#!/usr/bin/env python3
"""Lawfully re-derive the claim inventory at theory-v2.1-grounded-canon.
...
anchor, prior-inventory base, and validation mode needed for the
v0_2 -> v2_1 migration.
"""
...
OUTPUT_DIR = REPO / "Grounding" / "v2_1"
EXTRACTOR_PATH = REPO / "Grounding" / "v0" / "extract_v0.py"

DOCTRINE_MUTATION_ATTRIBUTION = {
    "translation_path_composition": {
        "source_file": "Core/README.DME.V2.IdentityEnvelopeComposition.md",
        "section_fragment": "Translation Path Composition",
    },
    "identity_plurality": {
        "source_file": "Core/README.DME.V2.IdentityEnvelopeComposition.md",
        "section_fragment": "Identity Plurality",
    },
    "support_and_authorization": {
        "source_file": "Core/README.DME.V2.IdentityEnvelopeComposition.md",
        "section_fragment": "Support and Authorization",
    },
    "grounded_inquiry_protocol": {
        "source_file": "Agentic_Reasoning/README.DME.V2.GroundedInquiryProtocol.md",
        "section_fragment": None,
    },
    "authority_debt_anchor": {
        "source_file": "Regime_Ecologies/README.DME.V2.AuthorityReferenceEcology.md",
        "section_fragment": "AuthorityDebt",
    },
}


def load_extractor_module():
    spec = importlib.util.spec_from_file_location("dme_v2_1_extract_wrapper", EXTRACTOR_PATH)
...
def parse_fcl_claim_cluster_refs():
    ...
    for entry_name in ("FCL-001", "FCL-002"):
        start = text.find(f"### {entry_name}")
        if start == -1:
            continue
        next_heading = text.find("### FCL-", start + 1)
        next_major = text.find("## 5. Queued Correspondences", start + 1)
        stops = [index for index in (next_heading, next_major) if index != -1 and index > start]
        end = min(stops) if stops else None
        segment = text[start:end]
        refs[entry_name] = sorted(set(__import__("re").findall(r"CLAIM-[A-Z]+-\d{4}", segment)))
    return refs
...
def resolve_ref_chain(claim_id: str, live_ids: set[str], crosswalks: list[dict]) -> dict:
    ...
    for crosswalk in crosswalks:
        ...
        target = info.get("v0_2_id") or info.get("v2_1_id")
...
def validate_fcl_chain(entries_v2_1: list[dict], crosswalk_v2_1: dict) -> dict:
    live_ids = {entry["claim_id"] for entry in entries_v2_1}
    crosswalks = [
...
def normalize_new_entry_statuses(crosswalk: dict):
    for info in crosswalk.values():
        if info.get("status") == "new_in_v0_2":
            info["status"] = "new_in_v2_1"
...
def assign_stable_ids_relaxed(module, entries_new: list[dict], entries_old: list[dict]):
    ...
    crosswalk = {
        old["claim_id"]: {
            ...
            "v2_1_id": None,
            "reason": "No deterministic v2.1 successor matched on source_file, claim_type, overlap, or fallback verbatim/term similarity.",
        }
        for old in entries_old
    }
    consumed_old = set()
    ...
    for _, entry in indexed_entries:
        ...
        if candidates:
            ...
            crosswalk[keeper["claim_id"]] = {
                "status": "preserved",
                "v2_1_id": keeper["claim_id"],
                "reason": "Deterministic v2.1 match preserved via source_file, claim_type, and overlap/verbatim/term similarity.",
            }
            absorbed = []
            ...
            crosswalk[entry["claim_id"]] = {
                "status": "new_in_v2_1",
                "v2_1_id": entry["claim_id"],
                "reason": "No prior v0.2 entry matched deterministically; allocated above the v0.2 type ceiling.",
            }

    return entries_new, crosswalk, dict(next_serial)


def new_entry_attribution(entries_v2_1: list[dict], crosswalk: dict) -> dict:
    new_ids = {
        claim_id
        for claim_id, info in crosswalk.items()
        if info.get("status") == "new_in_v2_1"
    }
    attribution = {key: 0 for key in DOCTRINE_MUTATION_ATTRIBUTION}
    unattributed = []
    for entry in entries_v2_1:
        if entry["claim_id"] not in new_ids:
            continue
        matched = False
        for key, rule in DOCTRINE_MUTATION_ATTRIBUTION.items():
...
def populated_summary(manifest: dict, entries_v2_1: list[dict], prior_manifest: dict) -> str:
    ...
    lines = [
        ...
        "",
        "```text",
        f"prior_inventory: Grounding/v0_2/claim_inventory.jsonl",
        f"total_entries: {manifest['total_entries']}",
        f"counts_by_type: {counts}",
        f"new_in_v2_1: {manifest['id_counts']['new_in_v2_1']}",
        ...
        "",
        "```text",
        *[f"{key}: {value}" for key, value in manifest["new_entry_attribution"].items() if key != "unattributed_claim_ids"],
        "```",
        "",
        "## 4. Chain Validation",
        "",
...
def main():
    ...
    entries_v2_1, crosswalk, _ = assign_stable_ids_relaxed(module, module.raw_entries, prior_entries)
    entries_v2_1.sort(key=lambda entry: (entry["claim_type"], module.parse_claim_id(entry["claim_id"])[1]))

    cross_document, duplicate_nc = module.build_recurrence(entries_v2_1)
    relation_counts = Counter(
        entry.get("relation_type", "unspecified")
        for entry in entries_v2_1
        if entry["claim_type"] == "DEP"
    )
    use_mention_counts = Counter(entry.get("use_mention_flag", "unknown") for entry in entries_v2_1)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(EXTRACTOR_PATH, OUTPUT_DIR / "extract_v0_2.py")

    chain_validation = validate_fcl_chain(entries_v2_1, crosswalk)
    if not chain_validation["passed"]:
        raise SystemExit(
            "v2.1 chain validation failed: "
    ...
    manifest = {
        ...
        "counts_by_type": dict(sorted(Counter(entry["claim_type"] for entry in entries_v2_1).items())),
        "total_entries": len(entries_v2_1),
        ...
        "id_crosswalk": "Grounding/v2_1/id_crosswalk.json",
        "id_counts": {
            ...
            "new_in_v2_1": id_counts.get("new_in_v2_1", 0),
        },
        ...
        "new_entry_attribution": new_entry_attribution(entries_v2_1, crosswalk),
    ...
    write_jsonl(OUTPUT_DIR / "claim_inventory.jsonl", entries_v2_1)
    ...
    with (OUTPUT_DIR / "ClaimInventory.POPULATED.v2_1.md").open("w", encoding="utf-8") as handle:
        handle.write(populated_summary(manifest, entries_v2_1, prior_manifest))
