#!/usr/bin/env python3
"""DME v0.3 deterministic claim extractor.

Substrates exercised: Structure (parse), Ledger (record). Nothing else.
This implements the v0.3 audit-driven fixes without semantic inference.
"""
...
EXAMPLE_SECTION_RE = re.compile(
    r"((^|[ >])(illustrative example|language example|example|example only|not a populated entry|entry record shape|record shape)([ >]|$))",
    re.I,
)
EXAMPLE_LINE_RE = re.compile(
    r"(^\s*(language example|example)\s*:?\s*$)|(illustrative example|example only|not a populated entry|example:)",
    re.I,
)
CONTEXT_PREFIX_RE = re.compile(
    r"^(without\b|without distinctions\b|if absent\b|if missing\b|if omitted\b|if unresolved\b|when missing\b|when absent\b|when omitted\b|failure mode\b|in the absence of\b|under loss\b|if no\b|missing\b|absent\b|omitted\b|unresolved\b)",
    re.I,
)
REGISTRY_SURFACE = "Core/README.DME.V2.NonCollapseRegistry.md"
REGISTRY_ENTRY_HEADING_RE = re.compile(r"^###\s+(NCR-\d+)\s*$")
REGISTRY_FIELD_RE = re.compile(r"^\s*([a-z_]+):\s*(.*)$", re.I)
...
def enrich_support_fields(entry: dict) -> None:
    ...
    entry.setdefault("basis", "doctrine_occurrence")
...
def parse_nc_terms(text: str):
    stripped = text.strip()
    match = NC_INLINE.match(stripped)
    if match and not stripped.startswith("="):
        return [norm_term(match.group(1)), norm_term(match.group(2)), "non_equivalence"]
    for regex in (NC_ISNOT, NC_IMPLY, NC_COLLAPSE):
        match = regex.search(stripped)
        if match:
            return [norm_term(match.group(1)), norm_term(match.group(2)), "non_equivalence"]
    return [first_words(stripped, 8), "registry_canonical"]


def process_registry_entry_block(
    ...
    canonical_text_raw = None
    canonical_text_line = None
    field_map = {}
    for line_number, text in fence_lines:
        match = REGISTRY_FIELD_RE.match(text.strip())
        if not match:
            continue
        ...
        if key == "canonical_text":
            canonical_text_raw = value
            canonical_text_line = line_number

    if not canonical_text_raw or canonical_text_line is None:
        return

    instance_refs = [
        item.strip()
        for item in field_map.get("instance_refs", "").split(",")
        if item.strip()
    ]
    instances_value = field_map.get("instances", "0").strip()
    try:
        instances_count = int(instances_value)
    except ValueError:
        instances_count = len(instance_refs)
    ...
    make_raw_entry(
        ...
        canonical_text_line,
        canonical_text_line,
        offsets[canonical_text_line - 1],
        offsets[canonical_text_line - 1] + len(fence_lines[canonical_text_line - fence_lines[0][0]][1]),
        canonical_text_raw,
        parse_nc_terms(canonical_text_raw),
        field_map.get("scale_band", "sentence / doctrine_law").replace(" / ", "/"),
        section_context,
        True,
        f"pattern:NC-registry-canonical;extractor:{EXTRACTOR_VERSION}",
        extra={
            "basis": "registry_canonical",
            "registry_entry_id": registry_entry_id,
            "canonical_text_raw": canonical_text_raw,
            "canonical_text_normalized": normalized_verbatim(canonical_text_raw),
            "source_field": "canonical_text",
            "instance_refs": instance_refs,
            "registry_instances": instances_count,
            "registry_fully_admitted": field_map.get("fully_admitted", "").lower() == "yes",
            "canonical_home": field_map.get("canonical_home", ""),
            "use_mention_flag": "asserted",
        },
    )
    stats["registry_canonical_entries"] += 1
...
def parse_file(source_file: str):
    ...
    pending_registry_entry_id = None
    ...
    while index < len(lines):
        ...
        if raw.lstrip().startswith("```"):
            ...
            while index < len(lines) and not lines[index].lstrip().startswith("```"):
                ...
                index += 1
            if source_file == REGISTRY_SURFACE and pending_registry_entry_id:
                process_registry_entry_block(
                    pending_registry_entry_id,
                    source_file,
                    fence_lines,
                    section_context(),
                    offsets,
        ...
        if heading_match:
            ...
            if source_file == REGISTRY_SURFACE:
                registry_match = REGISTRY_ENTRY_HEADING_RE.match(stripped)
                pending_registry_entry_id = registry_match.group(1) if registry_match else None
            example_hint_pending = False
            core_law_pending = False
            index += 1
            continue
...
def identity_text(entry: dict) -> str:
    if entry.get("canonical_text_normalized"):
        return entry["canonical_text_normalized"]
    return normalized_verbatim(entry["verbatim_span"])
...
def score_match(new_entry: dict, old_entry: dict) -> tuple:
    ...
    verbatim_equal = int(identity_text(new_entry) == identity_text(old_entry))
...
def build_recurrence(entries_v0_2):
    doctrine_entries = [
        entry for entry in entries_v0_2
...
def basis_summary(entries):
    counts = defaultdict(Counter)
    for entry in entries:
        basis = entry.get("basis", "doctrine_occurrence")
        counts[basis][entry["claim_type"]] += 1
    summary = {}
    for basis, counter in counts.items():
...
def split_crosswalk_records(crosswalk):
    transition_map = {}
    new_entry_registry = {}
    for claim_id, info in crosswalk.items():
        if info.get("status", "").startswith("new_in_"):
...
def build_manifest(corpus, entries_v0_2, relation_counts, use_mention_counts, transition_map, new_entry_registry, fcl_validation):
    id_counts = Counter(info["status"] for info in transition_map.values())
    id_counts["new_in_v0_3"] = len(new_entry_registry)
    return {
        "source_snapshot": TAG,
        ...
        "v0_3_fixes": [
            ...
            "DEP relation typing",
            "crosswalk schema split",
        ],
        "basis_strata": basis_summary(entries_v0_2),
        "rejection_counts": {
        ...
        "id_stability": {
            ...
            "new_entry_registry": "Grounding/v0_2/new_entry_registry.json",
        ...
        "id_counts": {
            ...
            "new_in_v0_3": id_counts.get("new_in_v0_3", 0),
        },
...
def build_populated_summary(
    ...
    new_ids = manifest["id_counts"]["new_in_v0_3"]
    ...
    lines = [
        "# DME V2 - Claim Inventory (POPULATED, v0.3)",
        "",
        "```text",
        ...
        "",
        f"cross-entry recurrent terms (doctrine basis): {len(cross_document)}",
        f"duplicate verbatim NC laws (doctrine basis):  {len(duplicate_nc)}",
        f"basis strata:                {manifest['basis_strata']}",
        ...
        *[f"  {relation}: {count}" for relation, count in sorted(relation_counts.items())],
        ...
        f"  reclassified_basis: {manifest['id_counts'].get('reclassified_basis', 0)}",
        f"  new_in_v0_3: {new_ids}",
        "```",
        "",
        ...
        *[
        ...
        "",
        "```text",
        "v0.3 remains deterministic Structure + Ledger only",
        ...
        "",
        "```text",
        "v0.3 improvement != semantic understanding",
...
def main():
    ...
    shutil.copyfile(Path(__file__), GROUNDING_V0_2 / "extract_v0_3.py")

    manifest = build_manifest(corpus, entries_v0_2, relation_counts, use_mention_counts, transition_map, new_entry_registry, fcl_validation)
    ...
    with (GROUNDING_V0_2 / "new_entry_registry.json").open("w", encoding="utf-8") as handle:
